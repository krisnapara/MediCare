    ...
    db.query(
      ...
      (err2, result2) => {
        if (err2) {
          ...
          return res
            .status(500)
            .json({ message: "Gagal mengecek jadwal dokter" });
        }

        if (result2.length > 0) {
          return res
            .status(400)
            .json({ message: "This timeslot is already taken" });
        }
        ...
        db.query(
          ...
          (err3) => {
            if (err3) {
              ...
              return res
                .status(500)
                .json({ message: "Gagal membuat reservasi" });
            }
...
export const updateReservationStatus = (req, res) => {
  ...
  db.query(sql, [status, req.params.id], (err) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Gagal mengubah status reservasi" });
    }
...
export const deleteReservation = (req, res) => {
  ...
  db.query(sql, [req.params.id], (err) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Gagal menghapus reservasi" });
    }
